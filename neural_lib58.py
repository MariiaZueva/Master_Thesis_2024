...
class ConceptNetwork:
    ...
    def fit(
            ...
            n_epochs: int = 2000,
            loss_fn=torch.nn.CrossEntropyLoss(), nonlinearity=torch.nn.Sigmoid#ReLU
            #loss_fn=torch.nn.BCELoss(), nonlinearity=torch.nn.ReLU
    ):
        
        X = torch.tensor(X_df[list(self.attributes)].values).float()
        y = torch.tensor(y.values).long()

        X_train_shuffle, y_train_shuffle = shuffle(X, y, random_state=random.randint(0, 1000))

        #print(X, '\n\n', y)

        self._network = self._poset_to_network(self.poset, nonlinearity)

        #for i in range(0, len(self._network), 2):
        #    print(i)
        #    print(self._network[i].weight)

        #optimizer = torch.optim.SGD(self.network.parameters(), lr = 1)
        optimizer = torch.optim.Adam(self.network.parameters(), lr = 0.001, betas = (0.9, 0.999))
        
       # print(self.edge_weights_from_network()[(21, 62)])
    
        for it in range(n_epochs):
            #print()
            optimizer.zero_grad()
            y_pred = self.network(X_train_shuffle)
            #print(y_pred)
    ...
    def fit_2(
        ...
        for it in range(n_epochs):
            #print(self.edge_weights_from_network())
            optimizer.zero_grad()
            y_pred = self.network(X_train_shuffle)
            #print(y_pred)
            loss = loss_fn(y_pred, y_train_shuffle)
    ...
    def predict(self, X_df: 'pd.DataFrame[bool]') -> torch.Tensor:
        return [self.predict_proba(X_df), self.predict_proba(X_df).argmax(1)]
    ...
    def _poset_to_network(poset: POSet, nonlinearity: type = torch.nn.ReLU) -> 'torch.nn.Sequential':
        ...
        # i = 0
        # while i < len(layers):
        #     torch.nn.init.uniform_(layers[i].weight, a=0, b=1)
        #     i+=2
        model_sparse = torch.nn.Sequential(*layers)
        #print(model_sparse[2].weight)
        #torch.nn.init.uniform_(model_sparse.weight, a=0, b=1)
        return model_sparse
